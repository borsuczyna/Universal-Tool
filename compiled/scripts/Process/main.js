...
import { setProgress, setFinish, outputConsole } from "../Queue/main.js";
import { outputLog } from '../Log/main.js';
const regex = {
    moviepyProgress: /t:[a-zA-Z0-9% ]*\|[a-zA-Z0-9%# ]*\| *([0-9]*)\/([0-9]*) *\[[0-9]*:[0-9]*<([0-9]*:[0-9]*)/,
};
const messageFromProcess = function (message) {
    let type = null;
    if (message.startsWith('[INFO] ')) {
        ...
        let match = /frame= *([0-9]+)/.exec(message);
        if (!match)
            return;
        ...
        let match = /Output *#[0=9]*, *[a-zA-Z0-9]*, *to *'(.*)\\(.*)':/.exec(message);
        if (!match)
            return;
        let finish = match[2];
