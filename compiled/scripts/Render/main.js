...
export async function Render(path, dataPath, props = {}, isTemplate) {
    let html = await readFile(`${dataPath}\\${path}.ejs`);
    for (let work of renderWorks) {
        let found = html.match(work.regex);
        if (found)
            found = found[0];
        while (found) {
